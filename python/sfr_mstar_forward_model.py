...
def forward_model(minmass=8,maxmass=12,massstep=0.2,minsfr=-2,maxsfr=2,sfrstep=0.2,tmax=3,ntimestep=30):
    ...
    for i in range(len(mstar_mesh[:,0])):
        ...
        # add multiprocessing part here to run evolve galaxy in parallel
        # just trying to do this quick - haven't thought about how to take
        # both mass and SFR steps into account, so just doing mp on sfr steps
        evolve_pool = mp.Pool(mp.cpu_count())
        myresults = [evolve_pool.apply_async(evolve_galaxy,args=(mstar_mesh[i,j],sfr_mesh[i,j],tmax,ntimestep)) for j in range(len(sfr_mesh[0]))]
        evolve_pool.close()
        evolve_pool.join()
        evolve_results = [r.get() for r in myresults]
        ...
        for j in range(len(sfr_mesh[0])):
            ...
            lookbackt, redshift,logmstar,logsfr,mstar,sfr,mass_loss = evolve_results[j]
