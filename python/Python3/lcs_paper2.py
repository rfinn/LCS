...
def colormass(x1,y1,x2,y2,name1,name2, figname, hexbinflag=False,contourflag=False, \
    ...
    * alphagray
    ...
    if cumulativeFlag:
        t = plt.hist(x1, normed=True, cumulative=True,bins=len(x1),color=color1,histtype='step',lw=1.5, label=name1+' (%i)'%(n1))
        t = plt.hist(x2, normed=True, cumulative=True,bins=len(x2),color=color2,histtype='step',lw=1.5, label=name2+' (%i)'%(n2))
    else:
    ...
    if cumulativeFlag:
        t=plt.hist(y1, normed=True, cumulative=True,orientation='horizontal',bins=len(y1),color=color1,histtype='step',lw=1.5, label=name1)
        t=plt.hist(y2, normed=True, cumulative=True,orientation='horizontal',bins=len(y2),color=color2,histtype='step',lw=1.5, label=name2)
    else:
...
class gswlc_full():
    ...
    def cut_BT(self):
        btflag = self.gsw[BTkey] <= float(args.BT)
...
class lcsgsw(gswlc_base):
    ...
    def write_file_for_simulation(self):
        ...
        tabcols = [c1,c2,self.membflag,self.cat[BTkey],self.cat['ng'],self.cat['logSFR'],self.cat['logMstar'],self.cat['fcre1'],self.cat['fcnsersic1'],self.cat['Rd_1'],self.cat['DELTA_V']]
    ...
    def cut_BT(self):
        btflag = (self.cat[BTkey] <= float(args.BT)) & (self.cat['matchflag'])
...
class comp_lcs_gsw():
    ...
    def get_legacy_images(self,lcsflag=None,ssfrmin=None,ssfrmax=-11):
        ...
        for i in ids:
            ...
            plt.title("NSID {0}, dSFR={1:.1f}, BT={2:.1f}".format(self.lcs.cat['NSAID'][i],self.lcs.dsfr[i].self.lcs.cat[BTkey]))
    ...
    def compare_BT(self,nbins=15,xmax=.3):
        ...
        for i in range(len(xvars)):
            plt.hist(xvars[i],bins=mybins,color=colors[i],normed=True,\
                     histtype='stepfilled',lw=lws[i],alpha=alphas[i],zorder=orders[i])#hatch=hatches[i])
            plt.hist(xvars[i],bins=mybins,color=colors[i],normed=True,\
                     histtype='step',lw=lws[i],zorder=orders[i],label=labels[i])#hatch=hatches[i])
    ...
    def compare_BT_lowsfr_field_core(self,nbins=12,coreonly=False):
        ...
        labels = ['MM Low SFR Field','Low SFR LCS','Low SFR Field']
        if coreonly:
            labels = ['MM Low SFR Field','Low SFR LCS Core','Low SFR Field']
        allbins = [np.linspace(0,0.3,nbins),np.linspace(1,6,nbins),\
                   np.linspace(0.4,1.6,nbins),np.linspace(0,1,nbins),\
                   np.linspace(9.7,11,nbins),np.linspace(-1,1,nbins)]
        ...
        for col in range((ncols)):
            if col == 0:
                ...
                xvars = [self.gsw.cat['ng'],self.lcs.cat['ng']]                
            elif col == 2:
            ...
            for i in range(len(xvars)):
                ...
                plt.hist(xvar,color=colors[i],normed=True,bins=allbins[col],\
    ...
    def compare_BT_lowsfr_field(self,nbins=12,coreonly=False):
        ...
        for i in range(len(xvars)):
            ...
            plt.hist(xvars[i],color=colors[i],normed=True,bins=np.linspace(0,.3,nbins),\
                 histtype=histtypes[i],lw=lws[i],alpha=alphas[i],zorder=zorders[i],label=labels[i])#hatch=hatches[i])
        #plt.legend()
    ...
    def compare_BT_lowsfr_lcs_massmatch(self,nbins=12,coreonly=False,lcsflag=None):
        ''' compare the B/T distribution of field galaxies with low and normal SFR '''
        # select low SFR galaxies from field
        if coreonly:
            lcsflag = self.lcs.membflag #| self.lcs.infallflag
        ...
        #for i,col in enumerate(field_cols):
        xlabels = ['B/T','Sersic n','g-i','prob Sc','logMstar','$\Delta SFR$']
        labels = ['Normal Field','Normal Field, MM to low SFR LCS Core','Low SFR LCS Core']
        # BT
    ...
    def plot_dsfr_BT(self,nbins=15,xmax=.3,writefiles=False,nsersic_cut=10,ecut=1):
        nflag = (self.lcs.cat['ng'] < nsersic_cut)
    ...
    def get_legacy_images_BT(self,lcsflag=None,ncol=4,nrow=4,fignameroot='dsfr-bt-',figsize=(12,10)):
        ...
        for i in ids:
            ...
            plt.xticks([],[])
            plt.yticks([],[])
            plt.title("sSFR={0:.1f},dSFR={1:.1f},BT={2:.1f},n={3:.1f}".format(self.lcs.ssfr[i],self.lcs.dsfr[i],self.lcs.cat[BTkey][i],self.lcs.cat['SERSIC_N'][i]),fontsize=10)
            ...
            nplot += 1
        plt.savefig(fignameroot+str(nfig)+'.png')
    def core_getlegacy(self,figsize=(12,10)):
        '''get legacy images for core galaxies with low dsfr and high B/T  '''
        flag = self.lcs.membflag &  (self.lcs.cat['logMstar']> self.masscut)  & (self.lcs.ssfr > self.ssfrcut) &\
    ...
    def infall_getlegacy(self,figsize=(12,10)):
        '''get legacy images for core galaxies with low dsfr and high B/T  '''
    ...
    def ks_stats(self,massmatch=False):
        ...
        field = (self.gsw.cat['logMstar'] > self.masscut) & (self.gsw.ssfr > self.ssfrcut)
        mf = self.gsw.cat['logMstar'][field]
        sfrf = self.gsw.cat['logSFR'][field]
        dsfrf = sfrf - get_BV_MS(mf)
        zc = self.gsw.cat['Z_1'][field]
        if massmatch:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--BT', dest = 'BT', default = 0.3, help = 'B/T cut to use.')    
    ...
    lcsfile = homedir+'/research/LCS/tables/LCS-GSWLC-X2-NO-DR10-AGN-Simard2011-tab1-vizier-10arcsec-Tempel-2021Jan07.fits'            
